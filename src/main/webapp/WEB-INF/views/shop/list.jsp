			...
			<div id="container_box">
				<section id="content">
					...
					<c:if test="${paging.prev }">
						<span>[ <a href="/shop/list?c=${cateCode }&l=${level }&num=${(paging.num) -1 }">이전</a> ]</span>
					</c:if>
					<c:forEach begin="${paging.startPageNum }" end="${paging.endPageNum }" var="num">
							<span> 
								
								<c:if test="${paging.num != num }">
									<a href="/shop/list?c=${cateCode }&l=${level }&num=${ num }">${num}</a>
								</c:if>
								<c:if test="${paging.num == num }">
									<b>${num }</b>
								</c:if>
					...
					<c:if test="${paging.next }">
						<span>[ <a href="/shop/list?c=${cateCode }&l=${level }&num=${(paging.endPageNum) +1 }">다음</a> ]</span>
					</c:if>
					</div>
				</section>
